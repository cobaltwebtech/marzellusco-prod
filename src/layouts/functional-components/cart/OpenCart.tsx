...
const OpenCart: React.FC<OpenCartProps> = ({ className = "", quantity }) => {
  return (
    <div className="relative text-xl text-dark hover:text-primary dark:border-darkmode-border dark:text-white">
      ...
      {quantity ? (
        <div className="bg-red-700 text-white dark:bg-red-400 text-xs rounded-full p-1 absolute -top-1 md:-top-2 -right-3 md:-right-4 w-5 h-5 flex items-center justify-center">
          {quantity}
        </div>
      ) : null}
    </div>
